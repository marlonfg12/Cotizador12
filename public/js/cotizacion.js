 ...
 * ==============================================================================
 * Maneja la creación, edición, cálculo reactivo de impuestos (IVA y Retefuente),
 * desglose no editable del 50% (anticipo y saldo), generación vectorial de PDF
 * y despacho omnicanal (WhatsApp con código 57 y Correo SMTP).
 * ==============================================================================
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ----------------------------------------------------------------------------
  // 1. GESTIÓN DE LA TABLA DINÁMICA DE ÍTEMS (SIN COLUMNA INNECESARIA DE UNIDAD)
  // ----------------------------------------------------------------------------
  ...
  /**
   * Recalcula subtotales, descuentos, IVA, Retefuente, total y anticipos
  ...
  function calcularTotales() {
    ...
    const aplicarIVA = checkIVA.checked;
    const ivaPorcentaje = aplicarIVA ? Math.max(0, Number(ivaInput.value) || 0) : 0;
    const iva = (subtotal * ivaPorcentaje) / 100;

    // 5. Retención en la fuente (Retefuente)
    const aplicarReteFuente = checkRetefuente.checked;
    const retefuentePorcentaje = aplicarReteFuente
    ...
    const reteicaPorcentaje = aplicarReteICA
    ...
    // 8. Anticipo y saldo (50% institucional fijo no editable)
    ...
    // Estilos limpios para filas desactivadas sin tachar hermanos
  ...
  function construirPayload() {
    ...
    return {
      destinatario: {
        ...
        tipoCliente: document.getElementById('destinatarioTipoCliente').value,
  ...
  function llenarFormulario(c) {
    ...
    document.getElementById('destinatarioNombre').value = d.nombre || '';
    document.getElementById('destinatarioEmpresa').value = d.empresa || '';
    document.getElementById('destinatarioTelefono').value = d.telefono || '';
    document.getElementById('destinatarioEmail').value = d.email || '';
    document.getElementById('destinatarioDireccion').value = d.direccion || '';
    document.getElementById('destinatarioNIT').value = d.nit || '';
    document.getElementById('destinatarioTipoCliente').value = d.tipoCliente || 'persona';
    ...
    document.getElementById('servicio').value =
      c.servicio || 'Venta de Material de Construcción / Ornamentación';
    document.getElementById('detalleServicio').value = c.detalleServicio || '';
    document.getElementById('metodosPago').value =
      c.metodosPago || 'Transferencia bancaria\nCuenta No.123 4567 8901';
    ...
    document.getElementById('inicioProyecto').value =
      c.inicioProyecto || 'Una vez confirmado el anticipo';
  ...
  // ----------------------------------------------------------------------------
  // 4. LISTENERS Y EVENTOS DE INTERFAZ
  // ----------------------------------------------------------------------------
  ...
  clienteSelect.addEventListener('change', () => {
    ...
    if (cliente) {
      document.getElementById('destinatarioNombre').value = cliente.nombre || '';
      document.getElementById('destinatarioEmpresa').value = cliente.empresa || '';
      document.getElementById('destinatarioTelefono').value = cliente.telefono || '';
      document.getElementById('destinatarioEmail').value = cliente.email || '';
      document.getElementById('destinatarioDireccion').value = cliente.direccion || '';
      document.getElementById('destinatarioNIT').value = cliente.nit || '';
      document.getElementById('destinatarioTipoCliente').value = cliente.tipoCliente || 'persona';
    }
    calcularTotales();
  });
  ...
  if (btnGuardarClienteManual) {
    btnGuardarClienteManual.addEventListener('click', async () => {
      ...
      if (clienteExistente) {
        clienteSelect.value = clienteExistente.id;
        UI.showMessage(
          'info',
  ...
  itemsBody.addEventListener('click', async (e) => {
    // 1. Botón alternar entre modo catálogo y modo manual
    ...
    if (btnToggle) {
      ...
      if (enCatalogo) {
        // Cambiar a modo manual (el input reemplaza al select)
        ...
        // Cambiar a modo catálogo (el select reemplaza al input)
    ...
    // 2. Botón para guardar material manual directamente en el catálogo de productos
    ...
    if (btnGuardarProd) {
      ...
      if (prodExistente) {
        // Ya existe: vincular directamente sin duplicar
        ...
        UI.showMessage(
          'info',
          `El producto "${prodExistente.nombre}" ya existe en el catálogo. Se vinculó automáticamente a esta fila.`
        );
        calcularTotales();
        return;
      }
      ...
      try {
        ...
        // Cambiar la fila actual a modo catálogo con el producto seleccionado
        const selectFila = tr.querySelector('.item-producto');
        const btnToggleFila = tr.querySelector('.btn-toggle-modo');
        if (selectFila && descInput && btnToggleFila) {
          descInput.style.display = 'none';
  ...
  checkIVA.addEventListener('change', calcularTotales);
  retefuenteInput.addEventListener('input', calcularTotales);
  checkRetefuente.addEventListener('change', calcularTotales);
  ...
  // ----------------------------------------------------------------------------
  // 5. ENVÍO, DESPACHO Y PERSISTENCIA
  // ----------------------------------------------------------------------------
  ...
  /**
   * Guarda o actualiza la cotización con opción de generar y descargar PDF o solo persistir cambios
  ...
  async function guardarCotizacion({ descargarPdf = false } = {}) {
    ...
    /**
     * Guarda o actualiza la cotización y genera/actualiza el PDF en la carpeta /pdf del servidor
     */
    async function guardarCotizacion() {
      ...
      try {
        ...
        // Guarda el archivo PDF directamente en la carpeta /pdf del proyecto
        await PdfCotizacion.guardarPdfEnServidor(guardada, false);
        ...
        UI.showMessage(
          'exito',
          `Cotización ${guardada.folio} guardada con éxito y PDF almacenado en la carpeta /pdf.`
        );
    ...
    // Guardar cotización y PDF en carpeta
    if (btnGuardar) {
      btnGuardar.addEventListener('click', guardarCotizacion);
    }

    // Submit tradicional del formulario
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      await guardarCotizacion();
    });

    // Guardar solo el estado sin recargar ni descargar PDF
    if (btnGuardarSoloEstado) {
      btnGuardarSoloEstado.addEventListener('click', async () => {
        ...
        try {
          ...
          UI.showMessage(
            'exito',
            `Estado de ${cotizacionActual?.folio || 'la cotización'} actualizado a "${nuevoEstado.toUpperCase()}" correctamente sin descargar PDF.`
          );
        } catch (err) {
    ...
    // ----------------------------------------------------------------------------
    // DESPACHO INTELIGENTE POR WHATSAPP CON DESCARGA DE PDF Y COPIA AL REMITENTE
    // ----------------------------------------------------------------------------
    ...
    // Cambio reactivo del mensaje según el destinatario seleccionado
    ...
    btnWhatsApp.addEventListener('click', async () => {
      try {
        ...
        // 1. Asegurar y actualizar PDF en la carpeta /pdf del servidor
        ...
        // 2. Analizar teléfono del cliente
        ...
        // 3. Analizar teléfono de la empresa / remitente
        ...
        // 4. Preparar mensajes predeterminados
        const plantilla =
        ...
        // 5. Configurar opciones del modal
        if (waDescEmpresa) {
        ...
        if (isMobileCliente) {
          ...
          // No es celular válido (ej. teléfono fijo 5405546 o vacío)
    ...
    // ----------------------------------------------------------------------------
    // 6. GESTIÓN DE TIEMPO DE ENTREGA Y DÍAS HÁBILES (LUNES A SÁBADO)
    // ----------------------------------------------------------------------------

    /**
     * Determina si una fecha es día hábil (Lunes a Sábado).
     * 0 = Domingo (No hábil)
     */
    ...
    /**
     * Obtiene la fecha del día hábil N a partir de una fecha base
     * Regla de fin de semana:
     * - Si la cotización es Viernes (5), Sábado (6) o Domingo (0), el día 1 hábil es el Lunes.
     * - Si es Lunes a Jueves, el día 1 hábil es el día siguiente.
     */
    function obtenerFechaDiaHabil(fechaBase, nDia) {
      ...
      if (diaSemanaBase === 5) { // Viernes -> Día 1 es Lunes (+3)
        d.setDate(d.getDate() + 3);
      } else if (diaSemanaBase === 6) { // Sábado -> Día 1 es Lunes (+2)
        d.setDate(d.getDate() + 2);
      } else if (diaSemanaBase === 0) { // Domingo -> Día 1 es Lunes (+1)
        d.setDate(d.getDate() + 1);
      } else { // Lunes a Jueves -> Día 1 es día siguiente (+1)
        d.setDate(d.getDate() + 1);
      }
    ...
    /**
     * Cuenta cuántos días hábiles de producción transcurren entre fechaBase y fechaDestino
     */
    ...
    /**
     * Inicializa las fechas por defecto del calendario (2 a 5 días hábiles)
     */
    ...
    /**
     * Recalcula el texto de tiempoEntrega cuando el usuario cambia las fechas en el calendario
     */
    ...
    async function init() {
      try {
        ...
        if (editId) {
          ...
          // Nueva cotización con valores por defecto de la configuración
          if (configuracion) {
            ivaInput.value = configuracion.ivaPorcentajeDefault ?? 19;
            checkIVA.checked = configuracion.aplicarIvaDefault !== false;
            retefuenteInput.value = configuracion.retefuentePorcentajeDefault ?? 2.5;
            checkRetefuente.checked = configuracion.aplicarReteFuenteDefault !== false;
            if (reteicaInput) {
              reteicaInput.value = configuracion.reteicaPorcentajeDefault ?? 0.69;
            }
            if (checkReteICA) {
              checkReteICA.checked = configuracion.aplicarReteIcaDefault !== false;
            }
          }
